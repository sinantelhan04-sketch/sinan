...
const MainScreen: React.FC<MainScreenProps> = ({ onLogout, username }) => {
    ...
    return (
        <div className="w-full max-w-4xl mx-auto">
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 sm:p-8 relative min-h-[500px] flex flex-col">
                 <div className="flex justify-between items-start mb-8">
                    <div className="flex items-center">
                        <img src="https://www.aksadogalgaz.com.tr/img/kurumsal-kimlik/Aksa_Dogalgaz.jpg" alt="Aksa Doğalgaz Logo" className="h-12 w-auto" />
                        <div className="ml-4 hidden sm:block">
                            <h1 className="text-2xl font-bold text-gray-800 dark:text-white">Tesisat Sorgulama</h1>
                            <p className="text-sm text-gray-500 dark:text-gray-400">Yetkili Personel Ekranı</p>
                        </div>
                    </div>
                    <button 
                        onClick={onLogout}
                        className="text-xs sm:text-sm text-red-600 bg-red-50 hover:bg-red-100 dark:bg-red-900/20 dark:text-red-300 dark:hover:bg-red-900/40 font-medium py-2 px-4 rounded-lg transition-colors border border-red-200 dark:border-red-800"
                    >
                        Güvenli Çıkış
                    </button>
                </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                        <div className="relative flex-grow">
                             <input
                                ...
                                placeholder="Tesisat Numarası Girin (Örn: 100123456)"
                                disabled={loading}
                                className="w-full appearance-none rounded-lg border-2 border-gray-200 dark:border-gray-600 px-4 py-3 pl-11 text-gray-900 dark:text-white bg-gray-50 dark:bg-gray-700 focus:outline-none focus:border-blue-500 focus:bg-white dark:focus:bg-gray-600 transition-all font-mono text-lg"
                            />
                        ...
                        <button
                            ...
                            className="w-full sm:w-auto px-8 py-3 font-bold text-white bg-blue-600 rounded-lg hover:bg-blue-700 disabled:bg-blue-300 dark:disabled:bg-blue-900 disabled:cursor-not-allowed transition-all shadow-md hover:shadow-lg active:scale-95 flex items-center justify-center min-w-[120px]"
                        >
                            {loading ? (
                                <div className="flex items-center">
                         ...
                         {searchPerformed && !loading && (
                            <button 
                                onClick={handleClear}
                                className="w-full sm:w-auto px-4 py-3 font-semibold text-gray-600 bg-gray-100 rounded-lg hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600 transition-colors"
                            >
                                Temizle
                            </button>
                        )}
                ...
                {!loading && !foundCustomer && !error && !searchPerformed && (
                     <div className="flex-grow flex flex-col items-center justify-center text-gray-400 dark:text-gray-500 py-12 opacity-50">
                        <SearchIcon />
                        <p className="mt-4 text-sm">Sorgulama yapmak için tesisat numarası girin.</p>
                    </div>
                )}
                        ...
                        >
                            {mapEmbedUrl ? (
                                <div className="bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 p-1 rounded-xl shadow-md">
                                    ...
                                    <div className="relative w-full h-[400px] bg-gray-100 rounded-b-lg overflow-hidden">
                                        <iframe
                                            src={mapEmbedUrl}
                                            width="100%"
                                            height="100%"
                                ...
                                <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 p-4 rounded-lg flex items-center text-yellow-800 dark:text-yellow-200">
                                    ...
                                    <span>Bu kayıt için harita bilgisi oluşturulamadı. (Koordinat veya net adres eksik)</span>
                                </div>
                            )}
                        </div>
                    </div>
