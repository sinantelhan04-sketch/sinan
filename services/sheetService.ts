...
import type { Credential, Customer, UserActivityStat, Announcement, PhoneUpdateRequest } from '../types';
...
export const getReportedInstallations = async (): Promise<Customer[]> => {
    ...
    try {
        ...
        return customers.map((c: any) => ({
            installationNumber: c.installation_number,
            name: c.name,
            phone: c.phone,
            address: c.address,
            latitude: c.latitude,
        ...
        console.error("Hatalı tesisatlar çekilemedi:", e);
        return [];
    }
};

export const submitPhoneUpdateRequest = async (request: PhoneUpdateRequest): Promise<void> => {
    const client = ensureClient();
    try {
        const { error } = await client.from('phone_update_requests').insert([
            {
                username: request.username,
                user_full_name: request.userFullName,
                installation_number: request.installationNumber,
        ...
        // Tablo yoksa SQL önerisiyle birlikte hata fırlat
        if (e.code === '42P01' || (e.message && e.message.includes('does not exist'))) {
            const sql = `
create table if not exists public.phone_update_requests (
  id bigint generated by default as identity primary key,
...
-- RLS Politikaları
alter table public.phone_update_requests enable row level security;
create policy "Herkes ekleyebilir" on public.phone_update_requests for insert with check (true);
create policy "Herkes okuyabilir" on public.phone_update_requests for select using (true);
create policy "Herkes güncelleyebilir" on public.phone_update_requests for update using (true);
            `;
            throw new Error("Veritabanı Hatası: 'phone_update_requests' tablosu eksik. Lütfen Supabase SQL Editöründe şu kodu çalıştırın:\n\n" + sql);
        }
        throw e;
    }
};

export const getPendingPhoneUpdates = async (): Promise<PhoneUpdateRequest[]> => {
    const client = ensureClient();
    try {
        const { data, error } = await client
            .from('phone_update_requests')
        ...
        return data.map((d: any) => ({
            ...
            installationNumber: d.installation_number,
        ...
        console.error("Bekleyen onaylar çekilemedi:", e);
        return [];
    }
};

export const approvePhoneUpdate = async (requestId: number, installationNumber: string, newPhone: string): Promise<void> => {
    const client = ensureClient();
    try {
        // 1. Müşteri telefonunu güncelle
        const { error: updateError } = await client
            .from('customers')
        ...
        // 3. Hatalı bildirim loglarını temizle (error_reported = false yap)
        await client
            .from('search_logs')
            .update({ error_reported: false })
            .eq('installation_number', installationNumber);
...
export const rejectPhoneUpdate = async (requestId: number): Promise<void> => {
    const client = ensureClient();
    try {
        const { error } = await client
            .from('phone_update_requests')
