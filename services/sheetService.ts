...
export const createAnnouncement = async (announcement: Announcement): Promise<void> => {
    ...
    try {
        ...
        if (error) {
            // Tablo yok hatası (42P01: undefined_table) VEYA Client side "Could not find the table"
            if (error.code === '42P01' || error.message.includes('does not exist') || error.message.includes('Could not find the table')) {
                const sql = `create table public.announcements (
...
to authenticated
using (true);`;
                throw new Error(`Tablo Bulunamadı! Lütfen Supabase SQL Editöründe şu kodu çalıştırın:\n\n${sql}`);
            }
            throw new Error(error.message);
        }
    } catch (e: any) {
        // Beklenmeyen hata yakalama
        if (e.message && (e.message.includes('Could not find the table') || e.message.includes('does not exist'))) {
             const sql = `create table public.announcements (
  id bigint generated by default as identity primary key,
  title text not null,
  content text not null,
